...
if [ "$Chosen" = 'power off' ]; then
    ~/.config/dmenu/scripts/dmenu-prompt.sh "Confirm $Chosen?" 'sudo -A systemctl poweroff'
elif [ "$Chosen" = 'reboot' ]; then
    ~/.config/dmenu/scripts/dmenu-prompt.sh "Confirm $Chosen?" 'sudo -A systemctl reboot'
else
    exit 1
fi
